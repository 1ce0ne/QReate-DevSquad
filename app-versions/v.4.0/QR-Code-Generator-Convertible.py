...
def create_qr_pdf(filename, start_number, end_number):
    ...
    # Динамически рассчитываем размер страницы
    PAGE_WIDTH = (QR_COLUMNS * BACKGROUND_WIDTH + (QR_COLUMNS - 1) * PADDING) + 10 * mm  # 5 мм с каждой стороны
    PAGE_HEIGHT = (QR_ROWS * BACKGROUND_HEIGHT + (QR_ROWS - 1) * PADDING) + 10 * mm  # 5 мм с каждой стороны

    c = canvas.Canvas(filename, pagesize=(PAGE_WIDTH, PAGE_HEIGHT))

    current_number = start_number
    while current_number <= end_number:
        # Начальные координаты с отступом 5 мм от краев
        x_start = 5 * mm  # Отступ 5 мм от левого края
        y_start = PAGE_HEIGHT - 5 * mm - QR_ROWS * BACKGROUND_HEIGHT - (QR_ROWS - 1) * PADDING  # Отступ 5 мм от верхнего края
...
def generate_qr_codes():
    try:
        ...
        if os.path.exists(filename):
            ...
            if not response:
                return  # Если пользователь не хочет перезаписывать, прерываем выполнение
